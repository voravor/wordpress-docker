...
class batcache {
	...
	var $group   = 'knowsy'; // Name of memcached group. You can simulate a cache flush by changing this.
...
// Never auto batcache rest api 
 if ( strstr($_SERVER['REQUEST_URI'], 'api/knowsy') ) {
 	return;
 }
...
//add our namespace so incrementing clears front end keys
if(!$batcache->namespace = wp_cache_get('heleo_namespace')) {
	$batcache->namespace = 1;
	wp_cache_set('heleo_namespace', $batcache->namespace);
}
...
if ( isset( $_SERVER['QUERY_STRING'] ) )
	...
	'namespace' => $batcache->namespace //heleo added 12/13/2015
);
...
//var_dump(serialize($batcache->keys));die;
